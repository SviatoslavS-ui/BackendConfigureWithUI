...
public class BonusSetupPage extends ParentPage {
    ...
    public void setGameBetAmountAndWandering() {
        try {
            String betAmount = ConfigClass.getConfigValue("bet_amount");
            actionsElements.enterTextWhenClickable(gameBetAmount, betAmount);
            actionsElements.enterTextWhenClickable(gameWandering, "0");
            logger.info("Bet amount selected");
        }
        catch (IOException ex) {
            ex.printStackTrace();
            logger.error("Can`t get bet amount from file ...");
        }
    }
    ...
    public void attachPlayerToBonus() {
        ...
        actionsElements.waitAbit();
        actionsElements.forceButtonToClick(attachPlayerButton);
    }
